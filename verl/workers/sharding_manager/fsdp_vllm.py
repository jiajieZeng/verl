...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def update_params(self, updated_params, peft_config=None):
        ...
        if peft_config:
            if self.base_sync_done:
                ...
                # Remove old LoRA first if it exists
                old_lora_id = self.adapter_lora_ids.get('default')
                if old_lora_id is not None:
                    try:
                        if hasattr(self.inference_engine, 'llm_engine'):
                            self.inference_engine.llm_engine.remove_lora(old_lora_id)
                        elif hasattr(self.inference_engine, 'worker'):
                            self.inference_engine.worker.remove_lora(old_lora_id)
                        logger.info(f"[Single-LoRA] Removed old adapter 'default' with lora_int_id={old_lora_id}")
                    except Exception as e:
                        logger.warning(f"[Single-LoRA] Failed to remove old adapter 'default': {e}")
    ...
    def update_multi_lora_params(self, multi_lora_params: dict, peft_configs_dict: dict):
        ...
        for idx, (adapter_name, params) in enumerate(multi_lora_params.items()):
            ...
            # Use fixed lora_int_id for each adapter (base + index)
            # agent_lora_0 -> 1001, agent_lora_1 -> 1002, etc.
            lora_int_id = MULTI_LORA_BASE_ID + idx + 1
            
            # First, remove old LoRA with the same ID if it exists
            # This ensures we replace old weights with new weights
            old_lora_id = self.adapter_lora_ids.get(adapter_name)
            if old_lora_id is not None:
                try:
                    if hasattr(self.inference_engine, 'llm_engine'):
                        self.inference_engine.llm_engine.remove_lora(old_lora_id)
                        ...
                        self.inference_engine.worker.remove_lora(old_lora_id)
                    logger.info(f"[Multi-LoRA] Removed old adapter '{adapter_name}' with lora_int_id={old_lora_id}")
                except Exception as e:
                    logger.warning(f"[Multi-LoRA] Failed to remove old adapter '{adapter_name}': {e}")
