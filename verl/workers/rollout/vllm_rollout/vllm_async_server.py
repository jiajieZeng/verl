...
class AsyncvLLMServer(AsyncServerBase):
    ...
    async def wake_up(self, tags: Optional[list[str]] = None):
        # 🔧 先调用 worker 的 wake_up，让 sharding_manager 同步 LoRA adapter
        if self._workers:
            try:
                ...
                if self._lora_enabled:
                    ...
                    print(f"[AsyncvLLMServer] LoRA disabled, skipping LoRA sync")
            except Exception as e:
                import traceback
                print(f"[AsyncvLLMServer] Warning: Failed to sync LoRA via worker wake_up: {e}")
                traceback.print_exc()
        else:
            print(f"[AsyncvLLMServer] Warning: No workers initialized, skipping LoRA sync")
