...
from ver.hierarchy_rl.hrl.utils.logging_utils import PrettyPrinter

"""ReasonRLRayPPOTrainer（中文说明）

本文件实现了一个基于 Ray 的分布式 PPO 训练器，用于大语言模型的 RLHF/GRPO 等强化学习训练。

核心职责：
- 管理分布式 Worker（Actor、Critic、参考策略、奖励模型等）的初始化与生命周期。
- 构建训练数据流：生成（rollout）→ 评估（奖励、KL）→ 优势计算 → 参数更新（Critic/Actor）。
- 提供验证流程（validation），并在训练过程中定期保存检查点与记录指标。

与 VeRL 框架的关系：
- 继承自 VeRL 的 RayPPOTrainer 接口风格（本类中显式实现了与之类似的 API）。
- 依赖 VeRL 的 DataProto、核心算法（advantage、KL）、数据集与日志工具。

注意：
- 该实现支持多种优势估计器（GAE、GRPO、REMAX等），并据此决定是否启用 Critic。
- 支持参考策略 KL 约束（可在奖励中加入 KL 惩罚）。
"""
...
class ReasonRLRayPPOTrainer(RayPPOTrainer):
    def __init__(
        ...
        """初始化基于 Ray 的分布式 PPO 训练器（中文）

        说明：该训练器运行在驱动进程，负责调度分布式工作组，通过 RPC 协调生成、
        价值评估、优势计算与参数更新等阶段。

        参数：
        - config: OmegaConf 配置，包含训练/模型/资源/算法等参数。
        - tokenizer: 分词器，用于编码/解码文本。
        - role_worker_mapping: 角色到 Worker 类型的映射（ActorRollout、Critic、RefPolicy、RewardModel）。
        - resource_pool_manager: Ray 资源池管理器，负责 GPU 分配与池化。
        - ray_worker_group_cls: WorkerGroup 类，默认使用 RayWorkerGroup。
        - processor: 可选，多模态处理器。
        - reward_fn: 训练阶段使用的奖励函数（函数式/模型式）。
        - val_reward_fn: 验证阶段的奖励函数。
        - train_dataset, val_dataset, collate_fn, train_sampler: 数据与采样相关对象。
        - device_name: 设备名称（如 "cuda"）。
        """
    ...
    def _validate_config(self):
        config = self.config
        # 校验配置合法性（GPU 数量、batch 大小可整除性、并行策略/移除 padding 等）
        ...
        # 辅助函数：检查 micro_batch_size 与 micro_batch_size_per_gpu 互斥性
        # 新推荐使用 "*_micro_batch_size_per_gpu"，避免同时设置
        ...
        # Actor 配置检查：当不使用动态 batch 时，需要满足可整除与并行规模约束
        if not config.actor_rollout_ref.actor.use_dynamic_bsz:
        ...
        # FSDP 下若开启序列并行，需要启用 remove_padding
        ...
        print("[validate_config] 配置检查通过！")

    def _create_dataloader(self):
        """创建训练/验证 DataLoader（中文）

        - 构造 RLHF 数据集（训练/验证），并根据配置选择随机或顺序采样。
        - 计算总训练步数 total_training_steps，并注入到优化器配置，便于学习率调度。
        """
        ...
        # 将 total_training_steps 注入到 actor/critic 的优化器配置（便于调度）
    ...
    def _validate(self, do_sample: bool = False):
        ...
        - 使用 Actor 生成验证输出（可选采样），
        - 通过 val_reward_fn 计算得分与附加指标，
        - 聚合并返回以便日志记录/监控。
        """
        from collections import defaultdict
        reward_tensor_lst = []
    ...
    def fit(self):
        ...
        概览：生成（Actor）→ 旧策略对数概率/参考策略对数概率 →（可选）RM 评分 → 奖励/优势 →
        更新 Critic/Actor → 定期验证与保存检查点。
        """
            ...
            for batch_idx, batch_dict in enumerate(self.train_dataloader):
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer('adv', timing_raw):
                        ...
                        # 驱动进程上计算优势（根据配置选择 GAE/GRPO/REMAX 等）
